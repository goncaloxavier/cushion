...
- `src/routes/produtos/[slug]/+page.server.ts` and `+page.svelte` - product/category detail route with a text-first editorial layout: a short summary lead above the longer description, an optional separated resistance/maintenance paragraph, full-frame shared `StoreMediaGallery`, and quote/catalogue CTA.
...
- `src/routes/contacto/+page.server.ts` - contact form load/action: CSRF cookie, origin check, honeypot, server validation, and CRM submission (Postgres).
...
- `src/lib/builder/` and `schemaTypes/builder/` - versioned section contract, defaults, validation, standalone React editor, and shared Sanity schemas used by `sitePage` documents. The retired parallel `builderPage` persistence path has been removed; `sitePage` is the only free-page document source of truth.
...
- `src/lib/server/builder.ts` and `builder-preview.ts` - server-only media upload client plus signed, short-lived, httpOnly, iframe-only editor preview sessions.
...
- `tests/server-foundation.spec.ts` - CI-only Postgres integration coverage for customer verification/session/password-reset and real registration/login/logout routes, staff order status/note persistence, privacy requests/exports, operational incident deduplication/resolution, CRM deduplication, and editable settings; it self-skips locally when `DATABASE_URL` is absent.
...
- `src/lib/server/db.ts`, `src/lib/server/customer-auth.ts`, `src/lib/server/orders.ts`, `src/lib/server/email.ts`, and `src/lib/server/payment.ts` - server-only ecommerce foundation: bounded Postgres connection, customer scrypt auth/session/token handling, trusted order creation, Resend email attempts, and fail-closed Ifthenpay PayByLink adapter boundary.
- `src/lib/server/privacy.ts`, `src/routes/conta/(area)/privacidade/**`, and `src/routes/painel/privacidade/**` - customer privacy requests, portable exports, processing restriction, marketing withdrawal, and staff completion/erasure workflow.
- `src/lib/server/incidents.ts` and `src/routes/painel/incidentes/**` - deduplicated persistent operational alerts for checkout, payment-log, and email failures. Alerts surface on the dashboard and must be explicitly resolved by staff.
- `migrations/0001_commerce_foundation.sql` through `0013_operational_incidents.sql` and `scripts/db-migrate.ts` - explicit Postgres schema and advisory-lock migration runner. Production startup applies pending migrations before serving.
- `scripts/data-retention.ts`, `docs/data-retention.md`, and `docs/operations.md` - scheduled private-data cleanup plus the operating checklist for alerts, dependencies, and deployment.
- `src/lib/components/StoreMediaGallery.svelte` - shared product/case/blog/Loja detail gallery and lightbox (image and video items); locks background scroll while open and supports keyboard navigation.
...
- `src/lib/sanity.ts` - public cached Sanity client plus Visual Editing draft/stega clients and route-scoped site/product/store/case/blog queries for dataset `production`; checkout has a strict uncached/fail-closed store-content path.
- `src/lib/sanity-edit-attributes.ts` - lazy preview-only loader for Sanity edit attributes, keeping the visual-editing helper out of normal visitor route bundles.
...
- `src/lib/server/crm.ts` - private server-only Postgres writer for client profiles (`crm_client_profiles`) and form submissions (`crm_form_submissions`); dedupes profiles by normalized email in one atomic `insert ... on conflict`. Validation is per-source so catalogue needs an address and contact needs a message. (Naming note: "CRM" here is the Postgres-backed leads/profiles module — unrelated to the deleted legacy Sanity `crm` dataset.)
...
- `src/lib/server/app-settings.ts` - generic admin-editable key/value store backed by Postgres (`app_settings`, migration `0009`); plain-text values only, upsert via `on conflict (key) do update`. For low-sensitivity operational values (worst case of exposure is quota abuse, not customer/payment data) — never store customer/order data here.
- `src/lib/server/deepl-settings.ts` - DeepL-specific layer on `app-settings.ts`. `getEffectiveDeeplApiKey()` resolves a Postgres override before falling back to `DEEPL_API_KEY` from the environment; `checkDeeplKey()` validates a candidate key against DeepL's `getUsage()` before it's persisted; `translate.ts` caches its `DeepLClient` and only reconstructs it when the effective key string changes, so a saved override takes effect on the very next translation call with no restart.
- `src/routes/painel/definicoes/+page.server.ts` and `+page.svelte` - admin-only settings page (`canManageStaff` guard) to view DeepL key status/usage and swap the API key in-app when the free plan's 1M-lifetime-character limit is reached, without a redeploy.
...
- `sanity.config.ts` - single `/website` workspace Studio config for public website editing. There is no private CRM workspace: staff accounts, leads, and client profiles live only in Postgres.
- `sanity.structure.ts` - client-friendly Studio navigation for public website content.
...
- `schemaTypes/` - Portuguese Sanity document and object schemas for editable site content, product categories, Loja products, case studies, and blog posts. Localized short copy uses a compact two-line `localizedString` textarea and longer copy uses `localizedText`; EN/ES are hidden and filled by the translation pipeline. Keeping the established object type avoids migrations for existing content.
...
- Private CRM + backoffice source of truth: Railway Postgres via `DATABASE_URL`, tables `crm_form_submissions`, `crm_client_profiles`, `staff_users`, `staff_sessions`. There is no Sanity CRM dataset anymore — it was deleted after the Postgres migration.
...
- Keep `sanity.config.ts` and `sanity.cli.ts` aligned on project id. `sanity.cli.ts` defaults to the public `production` dataset; there is no private `crm` workspace.
...
- Keep `schemaTypes/index.ts` limited to `websiteSchemaTypes` — there is no private CRM schema to isolate from it anymore.
...
- The existing Decking video/calculator block remains an intentional product-specific presentation. Product categories can also add repeatable Sanity-backed content sections through `contentSections`: each section supports an image or video, optional title/text/button, `left`/`right`/`top` media placement, and the controlled `white`/`fog`/`mint`/`deep`/`blue` surface presets. Each section also accepts an optional `label` with a `labelStyle` of `caption` (default; a small accent-bordered line under the media, matching the legacy Decking video figcaption), `pill` (a glass badge overlaid on the media itself), or `eyebrow` (small caps above the section title). Mobile always places media before copy, and media uses a stable uncropped frame.
