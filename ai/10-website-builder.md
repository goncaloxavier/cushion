...
- Loja categories are first-class documents and use the same progressive inspector as other content.
  `Categoria` contains the name, stable address, and display order; `Produtos associados` shows live
  and pending assignments and opens a product directly when it must be moved. Deletion stays blocked
  until every associated change has been published.
...
Builder sections are not exclusive to free pages. They can sit at different paths:

- `sitePage.sections` — free pages, rendered by `/[...path]`.
- `siteLanding.<page>.sections` — the landing and fixed pages, rendered in their established route
  layouts.
- `<detailDocument>.sections` — product, Loja product, case-study, and blog detail pages.

The client-facing name for this shared editing surface is always **Conteúdo da página**. Do not call
it “conteúdo adicional” or give different page types different names for the same tool. “Secção”
means one individual block inside that content and remains appropriate for actions such as
`Adicionar secção`, moving, duplicating, hiding, and deleting a block.

Every designed fixed/detail page has one `builderManagedSection` in that same ordered stream. It is
the stable reference to the route's established design and canonical CMS fields, not copied fallback
content. Free blocks can move before or after it, and the designed area can be hidden, but it cannot
be duplicated or deleted. Opening it in the editor exposes the real fields already used by that
route. This preserves the approved design while making the whole visible page honest inside
**Conteúdo da página**.

`ManagedPageComposition` splits the stream around that designed area and renders the exact existing
route markup at its stored position. `BuilderPageRenderer` renders the free blocks on both sides.
...
media, pricing, galleries, and article fields remain where they already live and are never copied
into the marker.

The Inspector resolves the owning path and reuses one `SitePageSectionsEditor`; it writes only the
section array and never replaces the surrounding document. For fixed and detail routes, the editor
exposes the relevant array to the shared renderer as a temporary page-shaped view, so selection and
immediate preview use the same message contract as free pages. Anything that assumes sections are
always at the document root will be wrong for fixed pages.

The landing migration is generated by the pure `buildHomeSections` helper. Its four seeded sections
reuse the same public Svelte components as the legacy fallback for solutions, impact, cases, and
partners. This is deliberate: section-driven output must first match the approved landing design
exactly before client customization is allowed to diverge it.

Product documents previously stored optional media blocks in `contentSections`. That field is now a
hidden compatibility value, never a second client-facing editor. Reads normalize those blocks into
the shared section shape and the product route renders only the shared section renderer. The first
explicit save through `Conteúdo da página` persists that normalized stream and removes
`contentSections` from the draft. An unrelated product edit cannot trigger the migration, and an
explicitly emptied section list cannot resurrect the historical blocks.
...
- Publishing validates required page data and section structure before replacing the published
  document. Draft autosave remains permissive so an editor may finish an incomplete section.
- Publish rejects missing or duplicate section keys, repeated or invalid anchors, unsafe links,
  unsupported layout and typography values, unmuted autoplay, empty required media, invalid
  collection sources, and section item counts outside their supported bounds.
...
- Contract tests cover schema registration, safe field types, translation compatibility, singleton
  ids, protected server boundaries, renderer-version behavior, landing/free-page section validation,
  and validation before every publish path.
...
- Public renderer tests cover routes and responsive behavior without depending on mutable live Sanity
  content.
- `tests/home-sections.spec.ts` proves that the landing migration preserves the approved order,
  content, actions, and layouts, and that every managed route has one valid, stable designed area
  whose order and visibility survive normalization. `tests/section-parity.spec.ts` proves that
  legacy landing content and authored landing sections use the same public renderer instead of
  maintaining parallel markup.
