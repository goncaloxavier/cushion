...
- The former code-managed Decking video/calculator block has been migrated into the same Sanity-backed `contentSections` model used by other product categories. Preserve its existing content through Sanity data; do not restore a slug-gated frontend exception.
- Every product category may use repeatable Sanity-backed content sections. Each section accepts an image or video, optional title/text/button, a `left`/`right`/`top` arrangement, and a controlled accessible colour preset. Media must remain uncropped and must stack before text on mobile.
...
- The DeepL free plan's translation quota is finite. Admins can swap the active API key without a redeploy from `/painel/definicoes`; the pasted key is validated against DeepL before it's saved, and the override always takes precedence over the `DEEPL_API_KEY` environment fallback.
