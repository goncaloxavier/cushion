...
- Integration tests: `tests/sanity-contract.spec.ts` checks schema/query/page-copy/contact/social/legal/image/fallback alignment once in the desktop Playwright project because the file contract is viewport independent.
- E2E tests: `tests/routes.spec.ts` checks public routes, stable current-page navigation, language-safe links, overflow, detail links, collection images, pagination scroll, refresh scroll reset, contact form gating, contact/social/legal links, and 404 handling across desktop/mobile where the viewport matters.
