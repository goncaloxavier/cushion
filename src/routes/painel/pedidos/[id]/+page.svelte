...
<header class="painel-page-head">
  <div>
    <a class="painel-back" href="/painel/pedidos">← Voltar aos pedidos</a>
...
<form method="POST" action="?/setStatus" class="painel-panel">
  ...
  <div class="painel-form-row">
    <select id="rec-status" name="status" class="painel-select">
...
<div class="painel-panel">
  <h2 class="painel-panel-title">Dados do pedido</h2>
  <div class="painel-field-grid">
    ...
    {#if s.postalCode}
      <div class="painel-field">
        ...
        <p class="painel-field-value">{s.postalCode}</p>
      </div>
    {/if}
    {#if s.locality}
      <div class="painel-field">
        <span class="painel-field-label">Localidade</span>
        <p class="painel-field-value">{s.locality}</p>
      </div>
    {/if}
    <div class="painel-field">
    ...
    <div class="painel-field">
      ...
      <p class="painel-field-value painel-mono">{fmtDateTime(s.submittedAt)}</p>
    </div>
    {#if s.language}
      <div class="painel-field">
...
{#if s.profileId}
  <p class="painel-page-sub"><a href={`/painel/perfis/${s.profileId}`}>Ver perfil do cliente →</a></p>
{/if}
