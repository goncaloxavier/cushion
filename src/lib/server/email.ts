...
export const contactsRecipient = () =>
  runtimeEnv.CONTACTS_TO_EMAIL || runtimeEnv.ORDERS_TO_EMAIL || ''
...
export const logEmailFailure = (context: string, result: EmailSendResult) => {
  ...
  void recordOperationalIncident({
    fingerprint: context,
    category: 'email',
    severity: result.status >= 500 ? 'error' : 'warning',
    title: 'Falha no envio de email',
    detail: result.error,
    context: {operation: context, providerStatus: result.status},
  })
}
